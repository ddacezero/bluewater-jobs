...
const AddCandidateModal: FC = () => {
  const { state, dispatch, addCandidateToPipeline } = useApp();
  const mob = useMobile();

  const { showAddModal, jobs, isSaving } = state;
  ...
  const add = async () => {
    if (!form.name || !form.email || !form.jobId) return;
    await addCandidateToPipeline({
      fullName: form.name,
      email: form.email,
      jobId: Number(form.jobId),
      source: form.source,
      recruiterName: form.recruiter,
      rating: 3,
      notes: "",
    });
    setForm({
      ...
      >
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          <div className={`grid ${mob ? "grid-cols-1" : "grid-cols-2"} gap-3.5`}>
            <div>
              <label className="text-[11px] font-bold text-[var(--color-text-subtle)] uppercase tracking-wide block mb-1.5">
                Job Opening <span className="text-[var(--color-danger)]">*</span>
              </label>
              <select
                className={inputClass}
                value={form.jobId}
